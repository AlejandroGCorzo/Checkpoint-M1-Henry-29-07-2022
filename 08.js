...
function countDeep(arraicito) {
  ...
  let nBooleanos = 0;
  let nUndefined = 0;
  function milanga(array) {
    for (let i = 0; i < array.length; i++) {
      if (Array.isArray(array[i])) {
        nArray++;
        milanga(array[i]);
      }
      if (Number.isInteger(array[i])) nNumbers++;
      if (typeof array[i] === "string") nStrings++;
      if (typeof array[i] === "boolean") nBooleanos++;
      if (array[i] === undefined) nUndefined++;
    }
  }
  milanga(arraicito);
  return (((nArray - nNumbers) * nStrings) / nBooleanos) ** nUndefined;
}
